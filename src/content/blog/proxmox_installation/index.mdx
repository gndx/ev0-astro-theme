...
Il faut aussi activer, depuis le bios, la/les options de virtualisation, par exemple sur HP Elite Desk 800 G3, il faut faire F10 au démarrage pour accéder au Bios, puis dans `Advanced` cocher les cases `Virtualization Technology (VTx)` et `Virtualization Technology for Directed I/O (VTd)`.
