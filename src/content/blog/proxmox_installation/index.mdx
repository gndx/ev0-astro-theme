...
***NOTE :** Vous ne pouvez **attribuer** un matériel (USB, carte PCiE Coral ou autres) qu'à **une seule VM**, cela veut dire que votre clé Zigbee ne pourra être vu **QUE** sur la VM à laquelle vous l'avez associé.
...
**NOTE :** `Vendor/Device ID` permet de sélectionner un USB par son ID qui est unique, ce qui permet de le changer de port USB sans conséquence. `USB Port` permet de sélectionner un USB par son emplacement sur votre matériel.
