...
Vous pouvez modifier le canal dans le fichier `configuration.yaml` du dossier `Zigbee2MQTT` se trouvant dans le dossier `config` en ajoutant le code suivant :
```yaml
advanced: #Il y est déjà normalement
  channel: 25
```
