...
Vous pouvez modifier le canal dans l'onglet `Configuration` en passant en `YAML` via les trois petit points en haut a droite (ou dans le fichier `configuration.yaml` du dossier `Zigbee2MQTT` se trouvant dans le dossier `config` via FileEditor ou VScode par exemple) en ajoutant le code suivant :
```yaml
advanced:
  channel: 25
```
![Configurer le cannal dans Zigbee2MQTT](./img/z2mqtt_configuration_channel.png)
