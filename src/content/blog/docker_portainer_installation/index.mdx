...
***Astuces :** Dans un prochain article, je vais vous parler de [Portainer Agent](/blog/docker_portainer_agent_installation) qui vous permet depuis une seule instance Portainer d'en contrôler plusieurs. Dans ce cas, vous n'utiliserez toujours qu'un seul nœud Portainer et donc la limite des trois nœuds ne vous impacte plus.*
...
***Note :** En fonction de votre navigateur, il se peut qu'il vous informe que le site n'est pas sécurisé, c'est normale, car nous nous connectons via une adresse IP et non un nom de domaine.*
...
***Note :** Comme vous le voyez, il est possible de réinjecter une sauvegarde d'une précédente installation de Portainer. Je vous indique plus loin dans l'article comment faire une sauvegarde.*
...
***Note :** Vous n'avez pas besoin de changer l'image de votre `docker-compose.yml`, même si vous stoppez et relancez le conteneur, il lancera la version Buisness Edition malgré la présence de l'image CE dans le fichier.*
...
***Note :** En Comunity Edition, seul `Download backup file` est disponible.*
...
***Note :** Pas le choix de passer en ligne de commande, car depuis l'interface de Portainer, vous ne pouvez pas gérer Portainer, logique)*
