...
Je pars d'une Debian 12 fraichement installé sur un serveur avec un utilisateur n'ayant pas les droits `sudo` pour plus de securité.
Je pars du principe que vous avez déjà Debian 12 d'installé sur votre machine et que vous savez vous [connectez en SSH](../ssh_connexion/) à serveur.
...
Ajouter l'adresse du repertoire dans les sources.
```
echo \
...
### Docker Compose.
* Lancer/Executer un `docker-compose.yaml`
...
## Conclusion.
Vous voila avec Docker et Docker compose installés sur votre serveur, vous avez maintenant la possibilité de lancer des containeurs soit en ligne de commande soit via un fichier `docker-compose`.
